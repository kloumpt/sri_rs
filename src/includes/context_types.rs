...
impl ContextObject{
	...
	pub fn complete_config(&mut self){
		match self.get_param("images_base_filename") {
	        Some(_)=>(),
	        None => { self.set_param(String::from("images_base_filename"), String::from(IMAGES_BASE_FILENAME)); () }
	    }
	    match self.get_param("sounds_base_filename") {
	        Some(_)=>(),
	        None => { self.set_param(String::from("sounds_base_filename"), String::from(SOUNDS_BASE_FILENAME)); () }
	    }
	    match self.get_param("texts_base_filename") {
	        Some(_)=>(),
	        None => { self.set_param(String::from("texts_base_filename"), String::from(TEXTS_BASE_FILENAME)); () }
	    }

	    match self.get_param("images_associations_filename") {
	        Some(_)=>(),
	        None => { self.set_param(String::from("images_associations_filename"), String::from(IMAGES_ASSOCIATION_FILENAME)); () }
	    }
	    match self.get_param("sounds_associations_filename") {
	        Some(_)=>(),
	        None => { self.set_param(String::from("sounds_associations_filename"), String::from(SOUNDS_ASSOCIATION_FILENAME)); () }
	    }
	    match self.get_param("texts_associations_filename") {
	        Some(_)=>(),
	        None => { self.set_param(String::from("texts_associations_filename"), String::from(TEXTS_ASSOCIATION_FILENAME)); () }
	    }
	...
	pub fn index_details(&self, verbose : bool){
				...
				Some(images_base_filename)=>{
					...
					if verbose {
						...
						for i in 0..descriptor.get_histogram().len(){
							println!("{}: {}", i, descriptor.get_histogram()[i]);
						}
					}
				},
				None=>()
		...
		for descriptor in self.get_sounds_base(){
			match self.get_sounds_associations().get(descriptor.get_id()){
				...
				None=>	println!("{}=>???", descriptor.get_id())
			}
		}
		println!("");
