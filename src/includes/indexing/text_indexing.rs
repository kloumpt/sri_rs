...
impl TextDescriptor{
	...
	pub fn from_text_file(context: &mut ContextObject, text_file: File) ->Result<TextDescriptor, String>{
		...
		let mut values: Vec<(String, u32)>=occurences.into_iter().collect();
		...
		Ok(TextDescriptor::from_occurences(context.gen_id(String::from("txt")), word_in_file, word_filtered, occurences))
	}
}


fn clean_string(value: String)->String{
	// Extrait de http://stackoverflow.com/a/4328722
	Regex::new("[.,/#!$%\\^&\\*;:{}=-_`~()]").unwrap().replace(&value, " ")
}
