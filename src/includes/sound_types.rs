...
impl SoundDescriptor{
	pub fn new(id: String)->SoundDescriptor{
		SoundDescriptor{
			id: id,
			histograms_size: 0,
			histograms_levels: 0,
	...
	pub fn from_histogram(id: String,  histograms_size: usize, histograms_levels: usize, histograms: Vec<Vec<u32>>)->SoundDescriptor{
		SoundDescriptor{
			id: id,
			histograms_size: histograms_size,
			histograms_levels: histograms_levels,
	...
	pub fn from_file(file_reader: &mut BufRead)->Result<Option<SoundDescriptor>, String>{
		...
		let mut histograms_size: usize = 0;
		let mut histograms_count: usize = 0;
	    let mut histograms_levels: usize = 0;
	...
	pub fn get_histograms_size(&self)->usize{
		self.histograms_size
	}

	pub fn get_histograms_levels(&self)->usize{
		self.histograms_levels
	}

	pub fn get_histograms(&self)->&Vec<Vec<u32>>{
		&self.histograms
	}
	...
	pub fn compare_to(&self, other: &SoundDescriptor) -> f64 {
		...
		let window_size = self.get_histograms_size();
		let window_levels = self.get_histograms_levels();


		let mut min_diff: f64=0.0;
		for i in 0..(histograms_amount_a - histograms_amount_b + 1) {
			for j in 0..histograms_amount_b{
				let mut diff_sum: f64=0.0;
				for (index, (value_a, value_b)) in self.get_histograms()[i+j].iter().zip(other.get_histograms()[j].iter()).enumerate(){
					if index >= window_levels{
						break;
					}
					diff_sum+=(*value_a as f64 - *value_b as f64).abs();
				}
				min_diff=min_diff.min(diff_sum);
			}
		}
		min_diff/=window_size as f64;
		return (1.0-min_diff)*100.0;
	}
}
